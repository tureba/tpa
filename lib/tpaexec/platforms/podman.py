...
from ..exceptions import PodmanPlatformError
...
class podman(Platform):
    ...
    def _validate_ccache(self, shared):
        ...
        ccache. By default, this is a named Podman volume shared between the
        ...
        if shared:
            ...
            if not os.path.isdir(ccache):
                try:
                    ...
                    raise PodmanPlatformError(f"--shared-ccache: {str(e)}")
        else:
    ...
    def _validate_sources(self, sources):
        ...
        Args:
            sources: We accept a list of "name:/host/dir/[:/container/dir[:flags]]" arguments here,
                     and transform it into a list of podman volume definitions.
    ...
    def image(self, label, **kwargs):
        ...
        * a podman image, e.g. "tpa/debian"
        * a podman image with a version, e.g. "tpa/debian:11".
    ...
    def update_instance_defaults(self, instance_defaults, args, **kwargs):
        y = self.arch.load_yaml("platforms/podman/instance_defaults.yml.j2", args)
        if y:
            if self.ccache:
    ...
    def process_arguments(self, args):
        ...
        podman_images = args.get("podman_images")
        if podman_images:
            s["podman_images"] = podman_images
